...
public:
    ...
    base_last_globalpos_time_(0, 0, RCL_ROS_TIME)
  {
    ...
    world_->AddUpdateCallback(StageRos2::stage_update_cb, this);
    world_->ForEachDescendant(StageRos2::find_model_cb, this);
  }
...
private:
  ...
  static int stage_update_cb(Stg::World *, void * user)
  {
    auto * node = static_cast<StageRos2 *>(user);
    node->world_callback();
    // return false;
    return 0;
  }
  ...
  static int find_model_cb(Stg::Model * mod, void * user)
  {
    auto * node = static_cast<StageRos2 *>(user);
